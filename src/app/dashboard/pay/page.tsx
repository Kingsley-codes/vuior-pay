...
export default function PayPage() {
  ...
  const visible = useMemo(() => bills.filter(bill => tab === "due" ? ["active", "upcoming"].includes(bill.status.toLowerCase()) && daysUntil(bill.dueDate) <= 30 : ["in review", "paid", "completed"].includes(bill.status.toLowerCase())).sort((a,b) => +new Date(a.dueDate) - +new Date(b.dueDate)), [bills, tab]);
  ...
  return <DashboardShell><div className="mx-auto max-w-[1320px] p-5 sm:p-7 lg:p-8">
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
      <section className="overflow-hidden rounded-xl border border-[#e1e8e5] bg-white shadow-[0_8px_28px_rgba(25,55,47,.04)]"><div className="flex items-center justify-between p-5"><h2 className="text-[15px] font-bold">My Bills</h2><Link href="/dashboard/bills?addBill=1" className="flex items-center gap-2 text-[10px] font-semibold text-[#009b67]"><Plus size={16}/> Add bill</Link></div><div className="grid grid-cols-2 border-b border-[#e7ecea]">{[["due","Due Soon"],["paid","Paid"]].map(([value,label]) => <button onClick={() => {setTab(value as "due"|"paid");setSelected(new Set())}} key={value} className={`h-12 border-b-2 text-[11px] font-semibold ${tab === value ? "border-[#009b67] text-[#009b67]" : "border-transparent text-[#718097]"}`}>{label} <span className="ml-1 rounded-full bg-[#eef3f1] px-2 py-0.5 text-[9px]">{bills.filter(b => value === "due" ? ["active","upcoming"].includes(b.status.toLowerCase()) : ["in review","paid","completed"].includes(b.status.toLowerCase())).length}</span></button>)}</div>
