...
export function JobApplicationForm({ job }: { job: JobDescription }) {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!hasCompletePhoneNumber(phone)) {
      setStatus({
        type: "error",
        title: "Application not submitted",
    ...
    setStatus({
      ...
      message:
        "Thanks for applying. Our careers team will review your details.",
    });
    form.reset();
    setPhone("");
  }
      ...
      {status ? (
        <div
          className={`fixed right-5 top-5 z-[9999] w-[min(420px,calc(100vw-40px))] rounded-[10px] border bg-white px-4 py-3 shadow-[0_18px_45px_#12332626] max-[620px]:right-4 max-[620px]:top-4 max-[620px]:w-[calc(100vw-32px)] ${
            status.type === "success" ? "border-[#9edfc9]" : "border-[#efb7b7]"
          }`}
          role="alert"
          aria-live="assertive"
        >
      ...
      >
        ...
        <label className="mt-5 flex items-start gap-3 text-[10px] leading-[1.6] text-[#667487] min-[901px]:text-[12px] max-[620px]:text-[13px]">
          ...
          <span>
            ...
            not a condition of purchase. Message and data rates may apply. See
            our{" "}
            <Link
              className="font-bold text-[#00895f] underline"
