...
export function useVuiorData(userId?: string) {
  ...
  useEffect(() => {
    ...
    const unsubscribeTransactions = onSnapshot(
      query(collection(db, "transactionHistory"), where("userId", "==", userId)),
      (snapshot) => {
        const next = snapshot.docs.map((item) => {
          const data = item.data();
          return {
            id: item.id,
            label: String(data.billName ?? data.name ?? data.description ?? "Bill payment"),
            category: String(data.category ?? "Payment"),
            amount: Number(data.amount ?? data.amountPaid ?? 0),
            status: String(data.status ?? "completed"),
            date: asDate(data.date ?? data.createdAt ?? data.paidAt) ?? new Date(0),
  ...
  const activeBills = useMemo(
    () => bills.filter((bill) => ["active", "upcoming"].includes(bill.status.toLowerCase())),
    [bills],
  );
